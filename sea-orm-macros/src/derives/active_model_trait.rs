...
fn derive_active_model_traits(ident: &Ident, all_fields: IntoIter<Field>) -> syn::Result<TokenStream> {
    ...
    Ok(quote!(
        ...
        #[automatically_derived]
        impl std::convert::From<Model> for #ident {
        ...
        #[automatically_derived]
        impl sea_orm::IntoActiveModel<#ident> for Model {
            fn into_active_model(self) -> #ident {
                self.into()
            }
        }

        #[automatically_derived]
        impl sea_orm::ActiveModelTrait for #ident {
            type Entity = Entity;
